...
export const RealityManipulationDossier: React.FC = () => {
  ...
  const paradigms: Record<'ar' | 'vr', RealityParadigm> = {
    ...
    vr: {
      ...
      examples: [
        ...
        { id: 33, domain: "Artistry", title: "Volumetric Spatial CAD Tools", description: "Infinite dimensional digital canvases where sculptors create life-size conceptual vehicles and designs using broad arm strokes.", icon: Zap },
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-10 space-y-8 antialiased selection:bg-cyan-500/30 text-slate-600 dark:text-slate-300">
      ...
      <div className="space-y-3 border-b border-slate-100 dark:border-slate-800 pb-6">
        ...
        <p className="text-sm md:text-base text-slate-400 dark:text-slate-500 max-w-2xl leading-relaxed">
          Reality manipulation technologies do not merely display structural metrics; they reshape sensory input. By blending digital constructs with physical frameworks or replacing our physical environment entirely, these structures re-map how we process spatial reality.
        </p>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeParadigm && (
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* Core Definition */}
              <div className={`lg:col-span-2 rounded-3xl border p-6 md:p-8 bg-white/40 dark:bg-slate-900/10 backdrop-blur-sm flex flex-col justify-between shadow-sm ${activeParadigm.accentColor} ${activeParadigm.glowColor}`}>
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-2xl font-black tracking-tight text-slate-950 dark:text-white">{activeParadigm.title}</h2>
                ...
                {/* Features */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8 pt-6 border-t border-slate-200/60 dark:border-slate-800/60">
              ...
              {/* Pipeline */}
              <div className="rounded-3xl border border-slate-200/60 dark:border-slate-800/80 bg-white dark:bg-slate-900/40 p-6 shadow-sm flex flex-col justify-between">
                <div>
                  ...
                  <div className="space-y-3 relative before:absolute before:left-[17px] before:top-4 before:bottom-4 before:w-0.5 before:bg-slate-100 dark:before:bg-slate-800">
                    {activeParadigm.pipeline.map((step, index) => (
                      <div key={index} className="flex items-start gap-3 relative group">
                        <div className="w-9 h-9 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex items-center justify-center text-xs font-mono text-slate-500 dark:text-slate-400 z-10 font-bold group-hover:border-slate-400 dark:group-hover:border-slate-600 transition-colors">
                          0{index + 1}
                        </div>
                ...
                <div className="mt-6 p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
            ...
            <div className="space-y-4">
              <div>
                <h3 className="text-lg font-black tracking-tight text-slate-950 dark:text-white">10 Concrete Implementations</h3>
                <p className="text-xs text-slate-400">Exhaustive operational real-world deployments of this paradigm.</p>
              </div>
                ...
                {activeParadigm.examples.map((ex) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`rounded-2xl border p-4 bg-white/80 dark:bg-slate-900/40 hover:bg-white dark:hover:bg-slate-900/60 cursor-pointer transition-all flex flex-col justify-between shadow-sm ${
                        isExpanded
                    ...
                    >
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-mono uppercase px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400">
                            {ex.domain}
                          </span>
                          <ex.icon className={`w-4 h-4 transition-transform ${isExpanded ? 'scale-110 text-slate-950 dark:text-white' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`} />
                        </div>
                        <h4 className="text-sm font-bold text-slate-950 dark:text-white leading-tight">
                          {ex.title}
                        </h4>
                        <p className={`text-xs text-slate-400 leading-relaxed ${isExpanded ? 'block pt-1' : 'line-clamp-2'}`}>
                          {ex.description}
                        </p>
                      </div>
            ...
            {/* Video */}
            <div className="aspect-video rounded-2xl overflow-hidden border border-slate-200/60 dark:border-slate-800/80 bg-black shadow-md">
              <iframe
                className="w-full h-full opacity-90"
                src={activeParadigm.videoURL}
        ...
        {activeTab === 'comparison' && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -15 }}
            transition={{ duration: 0.25 }}
            className="rounded-[2rem] border border-slate-200/60 dark:border-slate-800/80 bg-white dark:bg-slate-900/40 backdrop-blur-md shadow-sm overflow-hidden"
          >
            <div className="p-6 md:p-8 border-b border-slate-100 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/20">
              <h3 className="text-xl font-bold text-slate-950 dark:text-white">Structural Reality Matrix</h3>
              <p className="text-xs text-slate-400">Architectural differences across physical and sensory attributes.</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-sm">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-950 text-xs font-mono uppercase tracking-wider text-slate-500 dark:text-slate-400">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                  <tr className="bg-white dark:bg-transparent">
                    <td className="p-4 md:p-5 font-bold text-slate-950 dark:text-white">Primary Goal</td>
                  ...
                  <tr className="bg-slate-50/30 dark:bg-transparent">
                    <td className="p-4 md:p-5 font-bold text-slate-950 dark:text-white">Hardware Core</td>
                  ...
                  <tr className="bg-white dark:bg-transparent">
                    <td className="p-4 md:p-5 font-bold text-slate-950 dark:text-white">User Cognition</td>
                  ...
                  <tr className="bg-slate-50/30 dark:bg-transparent">
                    <td className="p-4 md:p-5 font-bold text-slate-950 dark:text-white">Interaction Style</td>
                  ...
                  <tr className="bg-white dark:bg-transparent">
                    <td className="p-4 md:p-5 font-bold text-slate-950 dark:text-white">Tracking Dependency</td>
            ...
            <div className="p-5 bg-slate-50 dark:bg-slate-950 text-xs text-center text-slate-400 dark:text-slate-500 border-t border-slate-100 dark:border-slate-800/60">
