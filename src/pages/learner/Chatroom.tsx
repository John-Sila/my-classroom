...
// Helper to verify if the chatroom is currently open (7:00 AM to 9:00 PM)
const isChatWindowOpen = (): boolean => {
  const now = new Date();
...
export default function Chatroom() {
    ...
    const [isTimeWindowOpen, setIsTimeWindowOpen] = useState(isChatWindowOpen());

    const bottomRef = useRef<HTMLDivElement | null>(null);
    const isTeacher = user?.rank === 'teacher';
    const lastCleanupRef = useRef<number>(0);

    // Periodically re-verify the active window status every minute
    useEffect(() => {
        const interval = setInterval(() => {
            setIsTimeWindowOpen(isChatWindowOpen());
        }, 60000);
        return () => clearInterval(interval);
    }, []);

    // Combine custom administrative settings with global operational hours restriction
    const isMessagingAllowed = canChat && isTimeWindowOpen;
    ...
    const sendMessage = async () => {
        // Enforce the operational time frame during function execution context
        if (!input.trim() || !user || !isMessagingAllowed) return;
        ...
        <div className="h-dvh w-full overflow-hidden bg-slate-100 p-3 text-slate-900 transition-colors duration-300 dark:bg-slate-950 dark:text-slate-100">
            ...
            <section className="flex min-w-0 flex-1 flex-col overflow-hidden border-r border-slate-200 dark:border-slate-800">
                ...
                <footer className="sticky bottom-0 shrink-0 border-t border-slate-200 bg-white/95 px-5 py-4 backdrop-blur-2xl dark:border-slate-800 dark:bg-slate-900/95">
                    ...
                    {canChat && !isTimeWindowOpen && (
                    <div className="mb-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-300">
                        The classroom chatroom is closed for the night. Operating hours are between 7:00 AM and 9:00 PM.
                    </div>
                    )}
                    ...
                    <textarea
                        ...
                        rows={1}
                        maxLength={500}
                        placeholder={isTimeWindowOpen ? "Broadcast a classroom message..." : "Chatroom closed until 7:00 AM"}
                        className={cn(
                    ...
                    <button
                        onClick={sendMessage}
                        disabled={!isMessagingAllowed || sending || !input.trim()}
                        className={cn(
